...
EIGEN_DECLARE_TEST(constexpr) {
    ...
    constexpr Matrix<int, Eigen::Dynamic, Eigen::Dynamic, 0, 3, 3> dyn_mat({{1, 2, 3},
                                                                            {4, 5, 6},
                                                                            {7, 8, 9}});
    VERIFY_IS_EQUAL(dyn_mat.size(), 9);
    VERIFY_IS_EQUAL(dyn_mat(0, 0), 1);
    static_assert(dyn_mat.coeff(0, 1) == 2);
    constexpr Array<int, Eigen::Dynamic, Eigen::Dynamic, 0, 3, 3> dyn_arr({{1, 2, 3},
                                                                           {4, 5, 6},
                                                                           {7, 8, 9}});
...
constexpr Matrix<int, 2, 2> global_mat({{1, 2},
                                        {3, 4}});
