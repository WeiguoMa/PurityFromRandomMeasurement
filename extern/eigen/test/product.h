...
inline bool verifyIsApprox(const Type1 &a, const Type2 &b, Tol tol) {
    ...
    if (!ret) {
        std::cerr << "Difference too large wrt tolerance " << tol << ", relative error is: "
                  << test_relative_error(a, b)
                  << std::endl;
    }
    return ret;
}
...
void product(const MatrixType &m) {
    ...
    */
    ...
    ColSquareMatrixType square2 = ColSquareMatrixType::Random(cols, cols), res2 = ColSquareMatrixType::Random(cols,
                                                                                                              cols);
    ...
    // regression for blas_trais
    {
        ...
        VERIFY(
                verifyIsApprox(square * (square * square).transpose(), square * square.transpose() * square.transpose(),
                               tol));
        ...
        VERIFY(
                verifyIsApprox(square * (square * square).conjugate(), square * square.conjugate() * square.conjugate(),
                               tol));
    }
    ...
    if (!MatrixType::IsRowMajor) {
        ...
        Map<RowSquareMatrixType, 0, Stride<Dynamic, 2> > map1(buffer.data(), rows, rows,
