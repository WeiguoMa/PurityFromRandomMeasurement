...
void svd_check_full(const MatrixType &m, const SvdType &svd) {
    ...
    enum {
        RowsAtCompileTime = MatrixType::RowsAtCompileTime, ColsAtCompileTime = MatrixType::ColsAtCompileTime
    };
...
void svd_compare_to_full(const MatrixType &m, const SvdType &referenceSvd) {
    ...
    SVD_STATIC_OPTIONS(MatrixType, Options)
    svd(m);
...
void svd_min_norm(const MatrixType &m) {
    ...
    // now check with SVD
    SVD_STATIC_OPTIONS(MatrixType2, Options)
    svd2(m2);
    ...
    SVD_STATIC_OPTIONS(MatrixType3, Options)
    svd3(m3);
...
void svd_inf_nan() {
    SVD_STATIC_OPTIONS(MatrixType, ComputeFullU | ComputeFullV)
    svd;
    typedef typename MatrixType::Scalar Scalar;
...
#pragma warning disable 239
    ...
    SVD_STATIC_OPTIONS(Matrix2d, ComputeFullU | ComputeFullV)
    svd;
    svd.compute(M);
...
#pragma warning pop
    ...
    SVD_STATIC_OPTIONS(Matrix3d, ComputeFullU | ComputeFullV)
    svd3;
...
void svd_preallocate() {
    ...
    SVD_STATIC_OPTIONS(MatrixXf, ComputeFullU | ComputeFullV)
    svd2(3, 3);
    internal::set_is_malloc_allowed(false);
    svd2.compute(m);
    internal::set_is_malloc_allowed(true);
...
void svd_verify_assert_full_only(const MatrixType &input = MatrixType()) {
    ...
    SVD_STATIC_OPTIONS(MatrixType, QRPreconditioner)
    svd0;
    ...
    SVD_STATIC_OPTIONS(MatrixType, QRPreconditioner | ComputeFullU)
    svdFullU(m);
    VERIFY_RAISES_ASSERT((svdFullU.matrixV()));
    VERIFY_RAISES_ASSERT((svdFullU.solve(rhs)));
    SVD_STATIC_OPTIONS(MatrixType, QRPreconditioner | ComputeFullV)
    svdFullV(m);
...
void svd_verify_assert(const MatrixType &input = MatrixType()) {
    ...
    SVD_STATIC_OPTIONS(MatrixType, QRPreconditioner | ComputeThinU)
    svdThinU(m);
    VERIFY_RAISES_ASSERT((svdThinU.matrixV()));
    VERIFY_RAISES_ASSERT((svdThinU.solve(rhs)));
    SVD_STATIC_OPTIONS(MatrixType, QRPreconditioner | ComputeThinV)
    svdThinV(m);
...
void svd_option_checks_full_only(const MatrixType &input) {
    ...
    SVD_STATIC_OPTIONS(MatrixType, QRPreconditioner | ComputeFullU | ComputeFullV)
    fullSvd(m);
    svd_check_full(m, fullSvd);
}
...
void svd_check_max_size_matrix(int initialRows, int initialCols) {
    ...
    svd_fill_random(m);
    SVD_STATIC_OPTIONS(MatrixType, QRPreconditioner | ComputeThinU | ComputeThinV)
    thinSvd(m);
    SVD_STATIC_OPTIONS(MatrixType, QRPreconditioner | ComputeThinU | ComputeFullV)
    mixedSvd1(m);
    SVD_STATIC_OPTIONS(MatrixType, QRPreconditioner | ComputeFullU | ComputeThinV)
    mixedSvd2(m);
    SVD_STATIC_OPTIONS(MatrixType, QRPreconditioner | ComputeFullU | ComputeFullV)
    fullSvd(m);
